...
class Settings {
  ...
  init() {
    ...
    if (argv.version) {
      settingsLog.info(`Doon Mist v${this.appVersion}`);
      process.exit(0);
    }
  ...
  get appName() {
    return this.uiMode === 'mist' ? 'Daon Mist' : 'Daon Wallet';
  }
  ...
  get rpcMode() {
    ...
    if (argv.rpc && argv.rpc.indexOf('ws:') === 0) {
      settingsLog.warn(
        'Websockets are not yet supported by Daon Mist, using default IPC connection'
      );
      argv.rpc = null;
      return 'ipc';
    } else return 'ipc';
  ...
  get rpcIpcPath() {
    ...
    if (process.platform === 'darwin') {
      ipcPath += '/Library/AndusChain/geth.ipc';
    } else if (
      process.platform === 'freebsd' ||
      process.platform === 'linux' ||
      process.platform === 'sunos'
    ) {
      ipcPath += '/.AndusChain/geth.ipc';
